...
/////////////////////////
//USERSSIGNUP ENDPOINT USING POST REQUEST + ENCRPT/HASHING THE PASSWORD 
// USERSCREATE
...
// When you use the POST request, you will create a user that will be added to the array(the database).
// Now create a post request on postman again 
// and see that your response for password is now hashed!


// CREATE LOGIN ROUTE/ENDPOINT AND ENCRPTED PASSWORD ONLY
...
//////AUTHENTICATION -TO CONFIRM that username&password used to log in matches the hashed password (NO generated authorization-TOKEN)
...
// CREATE LOGIN ROUTE, COMPARE PASSWORD + NO GENERATE AUTHORISATION-TOKEN
...
//         }
//     } catch {
//         res.status(500).send("other errors or user not allowed");
//     }
// });

// test the above code via postman,
// already create a user with the POST request to http://localhost:3000/usersSignup.
...
// if you change the password of the user in the body of the login request,It will respond with 'password incorrect'
...
// the authorization of user with JSON WEB TOKEN to enable or disable access to users logged in after authentication(login) & signup
...
// CREATE LOGIN ROUTE, COMPARE PASSWORD + GENERATE AUTHORISATION-TOKEN
//check if user exist
//then check if username & password match
        ...
        //if(await bcrypt.compare(req.body.password, user.password)) {
            bcrypt.compare(req.body.password, user.password, function(err, same){
                if(err){
                    ...
                    res.send("Success"); //generate authorisation-TOKEN=sucessful:username+password+token
                }else{
                    res.status(401).send("Username and Password Combination does not match")
                }
            })
    } catch {
        res.status(500).send("user not allowed"); //generate authorisation-TOKEN =unsucessful-token not created
    }
});


//////////////////////////
   //d.VERIFY AUTHORIZATION-TOKEN that was sent by creating a MIDDLEWARE that will:
...
// import the functions in your index.js file 
...
//TESTING POST REQUEST WITHOUT AUTHENTICATION
...
//CREATE USERS ROUTE+ GET REQUEST TO SEE ALL USERS
//create a route to ALL users 
app.get('/users',  authenticateToken, (req, res) => { 
    // res.send(users);  
    // or 
    response.status(200).send(users); //response with all the user
});

// TESTING THE GET REQUEST 
// To test this request, we will be using postman 
// Go to postman and on the drop down to the left, choose GET and set your url to http://localhost/3000/users 
...
//////////RENTAL ROUTE
///CREATE/ADD RENTAL
// So create and array of objects called rentals that will contain the username of the users logged in and their username, receiptNumber, ordersNumber,
// movieTitles,rentDate,returnDate in an object.
...
//////////////////////////////
//you have a rentals resource on your app that only users that are logged in and authenticated and verified can access that resource.

 app.get('/rentals', authenticateToken, (req, res) => { 
    res.send(rentals);   
});

// GET RENTAL VIA USERNAME & AUTHENTICATION
// Next, create a route that will handle GET requests for SPECIFIC rentals and
